...
<div style="padding-left:16px">
  ...
  <h1><center>Your pending requests</center></h1>
    <?php
	require_once("..//connection.php");
	
$userName = $_SESSION['userName'];
$sql = "SELECT * FROM requests WHERE (userName = '$userName' AND status = 'pending')";
$result1 = mysqli_query($conn, $sql);
if ($result1->num_rows > 0) {
// output data of each row
while($row1 = $result1->fetch_assoc()) {
	$itemID = $row1["itemID"];
	//view item details of request
...
?></p><center>No pending requests found.</center></p><?php
}
